...
export function getDisks(): DiskInfo[] {
    ...
    const enumerator = Gio
        .file_new_for_path("/dev/block")
        .enumerate_children("standard::symlink-target", null, null);

    for (const entry of enumerator) {
        const deviceName = entry.get_symlink_target()?.split("/")[1];
        if (!deviceName || deviceName.startsWith("loop") || !deviceName.match(/\d/)) continue;
        ...
        for (const line of lines) {
            ...
            if (device === devicePath) {
                return {
                    ...
                    used: (gtopFsusage.blocks - gtopFsusage.bfree) * gtopFsusage.block_size,
                    get usedDisplay() {
